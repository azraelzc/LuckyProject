...
local _pkgNameRefCountTable = {}
	...
	--预加载图集，公共资源库
local preLoadPackages = {"RoleIcon","EquipIcon","FamilyIcon","TextPic","Common1", "UIBattle"}

function UIManager.Init()
	logWarn("UIManager.Init----->>>")

	-- PgEventDispatch.RegisterCallback(PgEventID.OnEnterScene, this.OnEnterScene)
	-- PgEventDispatch.RegisterCallback(PgEventID.OnShowModalWait, this.ShowModalWait)
	-- PgEventDispatch.RegisterCallback(PgEventID.OnHideModalWait, this.HideModalWait)
	-- PgEventDispatch.RegisterCallback(PgEventID.OnShowModalBlack, this.ShowBlackModal)
	-- PgEventDispatch.RegisterCallback(PgEventID.OnHideModalBlack, this.HideBlackModal)
	-- PgEventDispatch.RegisterCallback(PgEventID.OnNetError, this.OnNetError)
 --    PgEventDispatch.RegisterCallback(PgEventID.OnUIMouseUp,this.OnUIMouseUp)
	return this
end
...
function UIManager.ShowPanel( panelType, callback)
	...
	local function tshow(ui)
		if ui.panelState == UIPanelState.Loaded then
            --打开任意非TIP类型窗口，关闭引导
            if panelType.uType ~= UIType.tip then
                UGManager.StopGuide()
            end
...
function UIManager.OnUIMouseUp(obj)
    --获取鼠标点击 Gobject
    for k,v in pairs(panelList) do
        if v ~= nil then
            if v.panelType ~= nil and v.panelType.clickOtherHide then
                if obj == nil then
                    v:hide()
                    --return
                end
                if v.view ~= nil and not v.view:IsAncestorOf(obj) then
                    v:hide()
                    --return
                end
            end
...
function UIManager.OnNetError(errorCode)
	logError("OnNetError:"..errorCode)
	CheckBoxMgr.ShowCheck(Language.Get("net_error1") ,function()
		Application.Quit()
    end)
end

function UIManager.OnSkillCD(skillIndex, skillCd)
	this.GetPanel(UIPanelType.Battle.name):OnSkillCD(skillIndex, skillCd)
end

function UIManager.OnLitHeadShow(guid)
	this.ShowPanel(UIPanelType.ActorLitHead, function(panel)
		panel:Refresh(guid)
		GRoot.inst:ShowPopup(panel.view,nil,nil,false)
	end)
end

function UIManager.OnActorInfoShow(guid)
	this.ShowPanel(UIPanelType.ActorInfoTip, function(panel)
		panel:Refresh(guid)
        --策划要求屏幕居中显示
		--GRoot.inst:ShowPopup(panel.view)
        panel.view:Center(true)
	end)
end

function UIManager.OnNpcClick(npcId)
	local npcLogic = LogicManager.Get(UIPanelType.TaskDialog.name)
	npcLogic.OnNpcClick(npcId)
end

function UIManager.OnEnterScene()
	this.ShowPanel(UIPanelType.Loading)
end

function UIManager.CreateGameObject(modelName, callBack)
	local onLoadEnd = function(object)
		if callBack ~= nil then
			callBack(object)
		end
	end
	resMgr:LoadGameObject(modelName, nil,nil, onLoadEnd)
end